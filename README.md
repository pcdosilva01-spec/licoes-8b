...
O banco precisa ser MySQL ou compatível com o protocolo MySQL, como PlanetScale ou TiDB Cloud, porque o schema usa Drizzle com `mysql2`. Para o TiDB Cloud, o projeto converte a `DATABASE_URL` em opções MySQL e configura `ssl: { minVersion: "TLSv1.2", rejectUnauthorized: true }` automaticamente. Não acrescente `?ssl=true`: no mysql2 esse valor booleano é inválido; o objeto SSL já é montado pelo código. Crie o banco vazio, coloque a URL limpa em `DATABASE_URL` e execute `npm run db:push` uma vez antes de usar o ambiente de produção. Nunca coloque a URL, a chave S3 ou `CRON_SECRET` no GitHub.
